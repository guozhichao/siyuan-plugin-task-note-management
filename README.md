...
为了可持续的维护插件，计划春节后（2026.02.24）对日历视图和项目看板功能进行收费：可试用7天，年付30，终身99，通过思源账户ID绑定会员。其他功能（任务管理侧栏、番茄钟、习惯打卡、四象限面板等等）依然完全免费。收费之后可以把开发成本平摊到每个付费用户上，每个人出一点钱，我就有动力继续去维护和优化插件。
...
验证是否是会员根据授权码本地验证，不依赖网络，只要输入ID和授权码就可以验证，并计算出会员到期时间
