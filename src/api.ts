...
export async function writeReminderData(data: any): Promise<any> {
    ...
    return putFile('data/storage/petal/siyuan-plugin-reminder/reminder.json', false, blob);
}

export async function readReminderData(): Promise<any> {
    try {
        const content = await getFile('data/storage/petal/siyuan-plugin-reminder/reminder.json');
        if (!content) {
            return {};
        }
